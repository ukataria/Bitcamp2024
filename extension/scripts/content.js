...
document.addEventListener('mouseover', function(event) {
    if (event.target.tagName === 'IMG') {
        ...
        const imageUrl = event.target.getAttribute('src'); // Replace this with your image URL
...
fetch('http://127.0.0.1:5000/analyze', data)
  ...
  .then(jsonResponse => {
    ...
    resultFromAPI = jsonResponse.result;
    confidenceAPI = (jsonResponse.confidence * 100).toFixed(2) + '%';
    x
  })
  .catch(error => {
    // Handle errors
    console.error('There was a problem with your fetch operation:', error);
        ...
        helloDiv.textContent = resultFromAPI + '' + confidenceAPI;
